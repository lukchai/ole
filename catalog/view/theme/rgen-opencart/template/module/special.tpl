...
?>
<?php if($products) {?>
<div class="box special-prd m-mod" id="special-prd<?php echo $module; ?>">
...
<?php if(isset($position)){ 
	...
	******************************/	
	if($position == 'column_left' || $position == 'column_right'){ ?>
		<div class="box special-prd<?php 
			if($this->config->get('RGen_colprdView_special') == "list1"){ ?> col-prd-thm1<?php }
			?>" id="special-prd<?php echo $module; ?>">
			<div class="box-heading header-1"><span><?php echo $heading_title; ?></span></div>

			<?php 
			/* Column product display in list
			******************************/	
			if($this->config->get('RGen_colprdView_special') != 'scroll'){ ?>
		...
		******************************/
		...
		if (
			...
			$this->config->get('RGen_SlideShow_Mod') == 'special' && 
			$getRoute == 'common/home') { ?>
			<div class="box special-prd slidshow-prd" id="slidshow-box">
				<div class="box-heading header-1"><?php echo $heading_title; ?></div>
				<div class="box-product owl-carousel">
					<?php foreach ($products as $product) { ?>
					<div class="item">
		...
		<?php }else { 
			...
			******************************/
			$modprd = $this->config->get('RGen_modPrdBlock_special');
			?>
			<div class="box special-prd<?php 
				...
				if ($this->document->RGen_modprdView[0] == 'special-grid'){ ?> mod-grid<?php }

				?>" id="special-prd<?php echo $module; ?>">
				<div class="box-heading header-1"><?php echo $heading_title; ?></div>
				<div class="box-product<?php echo $this->document->RGen_modprdView[0] == 'special-scroll' ? ' owl-carousel' : null; ?>">
					<?php foreach ($products as $product) { ?>
					<div class="item">
						<?php 
			...
			<?php 
			if($this->document->RGen_modprdView[0] == 'special-scroll'){ 
			?>
			<script type="text/javascript"><!--
			$(document).ready(function(){
				var obj = '#special-prd<?php echo $module; ?>';
