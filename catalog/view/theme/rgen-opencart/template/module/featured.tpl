...
?>
<?php if($products) {?>
<div class="box featured-box m-mod" id="featured-box<?php echo $module; ?>">
...
<?php if(isset($position)){
	...
	******************************/	
	if($position == 'column_left' || $position == 'column_right'){ ?>
		<div class="box featured-box<?php 
			if($this->config->get('RGen_colprdView_featured') == "list1"){ ?> col-prd-thm1<?php }
			?>" id="featured-box<?php echo $module; ?>">
			<div class="box-heading header-1"><?php echo $heading_title; ?></div>
			
			<?php 
			/* Column product display in list
			******************************/	
			if($this->config->get('RGen_colprdView_featured') != 'scroll'){ ?>
			...
			******************************/	
			...
			<script type="text/javascript">
				$(document).ready(function() {
					var obj = '#featured-box<?php echo $module; ?>';
		...
		******************************/
		...
		if (
			...
			$this->config->get('RGen_SlideShow_Mod') == 'featured' && 
		...
		<?php } else { 
			...
			******************************/
			$modprd = $this->config->get('RGen_modPrdBlock_featured');
			?>
			<div class="box featured-box<?php 
				...
				if ($this->document->RGen_modprdView[1] == 'featured-grid'){ ?> mod-grid<?php } 

				?>" id="featured-box<?php echo $module; ?>">

				<div class="box-heading header-1"><?php echo $heading_title; ?></div>
				<div class="box-product<?php echo $this->document->RGen_modprdView[1] == 'featured-scroll' ? ' owl-carousel' : null; ?>">
					<?php foreach ($products as $product) { ?>
					<div class="item">
						<?php 
			...
			<?php 
			if($this->document->RGen_modprdView[1] == 'featured-scroll'){ 
			?>
			<script type="text/javascript"><!--
			$(document).ready(function(){
				var obj = '#featured-box<?php echo $module; ?>';
