...
?>
<?php if($products) {?>
<div class="box bestseller-prd m-mod" id="bestseller-prd<?php echo $module; ?>">
...
<?php if(isset($position)){ 
	...
	******************************/	
	if($position == 'column_left' || $position == 'column_right'){ ?>
		<div class="box bestseller-prd<?php 
			if($this->config->get('RGen_colprdView_best') == "list1"){ ?> col-prd-thm1<?php }
			?>" id="bestseller-prd<?php echo $module; ?>">
			<div class="box-heading header-1"><?php echo $heading_title; ?></div>

			<?php 
			/* Column product display in list
			******************************/
			if($this->config->get('RGen_colprdView_best') != 'scroll'){ ?>
			...
			******************************/	
			...
			<script type="text/javascript">
				$(document).ready(function() {
					var obj = '#bestseller-box<?php echo $module; ?>';
		...
		******************************/
		...
		if (
			...
			$this->config->get('RGen_SlideShow_Mod') == 'best' && 
			$getRoute == 'common/home') { ?>
			<div class="box bestseller-prd slidshow-prd" id="slidshow-box">
				<div class="box-heading header-1"><?php echo $heading_title; ?></div>
				<div class="box-product owl-carousel">
					<?php foreach ($products as $product) { ?>
					<div class="item">
		...
		<?php }else { 
			...
			******************************/
			$modprd = $this->config->get('RGen_modPrdBlock_best');
			...
			<div class="box bestseller-prd<?php 
				...
				if ($this->document->RGen_modprdView[2] == 'best-grid'){ ?> mod-grid<?php } 

				?>" id="bestseller-prd<?php echo $module; ?>">
				
				<div class="box-heading header-1"><?php echo $heading_title; ?></div>
				<div class="box-product<?php echo $this->document->RGen_modprdView[0] == 'best-scroll' ? ' owl-carousel' : null; ?>">
					<?php foreach ($products as $product) { ?>
					<div class="item">
						<?php 
			...
			<?php 
			if($this->document->RGen_modprdView[2] == 'best-scroll'){ 
			?>
			<script type="text/javascript"><!--
			$(document).ready(function(){
				var obj = '#bestseller-prd<?php echo $module; ?>';
				$(obj+ " .owl-carousel").owlCarousel({
